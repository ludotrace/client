...
- **Auto-update** — `internal/version/version.go`: `Version` var injected via ldflags. `internal/updater/updater.go`: `Updater.Check()` fetches `https://ludotrace.com/client/version.json`, compares semver, returns an `*Update` if newer; `Updater.Stage()` streams the binary, verifies SHA-256, renames atomically to `<configDir>/pending_update[.exe]`. `ManifestURL` compiled in via ldflags. `cmd/ludotrace/main.go`: `--finish-update <original-path>` flag self-installs on launch; `runUpdateWorker` goroutine checks on startup then on a server-controlled interval; re-surfaces any pre-existing `pending_update` file. `internal/tray/tray.go`: `NotifyUpdateReady()` shows "Update vX.Y ready" label + "Restart to Update" item. `.github/workflows/release.yml`: on `v*` tag — builds all 4 platforms, computes sha256, creates GitHub Release, checks out site repo, copies binaries to `downloads/`, writes `client/version.json`, commits and pushes. Correct by inspection; not tested end-to-end (no tagged release yet, no site deploy token configured).
