...
- **Auto-update** — `SPEC.md` → Auto-Update section specifies the full flow: version manifest at `https://ludotrace.com/client/version.json`, `internal/updater` package with `Updater.Check()` + `Updater.Stage()`, SHA-256 verification, `pending_update[.exe]` staged binary, tray "Restart to Update" item, `--finish-update <original-path>` self-install flag, and a GitHub Actions release pipeline. Neither `internal/updater/` nor `internal/version/` packages exist. The `version` variable lives as a bare `var version = "dev"` in `cmd/ludotrace/main.go:24` rather than the specced `internal/version` package. No ldflags wiring for `ManifestURL`. Tracked in client issue #6.
